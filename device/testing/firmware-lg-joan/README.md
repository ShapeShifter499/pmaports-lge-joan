...
There is no `owner-firmware-lg-joan.tar`. If a build asks for that file, the
recipe is stale — use this package (pkgrel 8+), not
`ShapeShifter499/firmware-lg-joan`.
...
`firmware-lg-joan-blobs` at the commit pinned in `APKBUILD`. A missing
`owner-firmware-lg-joan.tar` means you copied an old recipe.
